...
contract SimpleGeneralMessage {
    ...
    function sendMessage(string memory message, address destAddress, uint16 destChainId) external payable {
        // Wormhole recommends that message-publishing functions should return their sequence value
        _sendMessageToRecipient(destAddress, destChainId, message, 0);
    }
    ...
    function _sendMessageToRecipient(address recipient, uint16 _chainId, string memory message, uint32 nonce) private returns (uint64) {
        bytes memory payload = abi.encode(recipient, _chainId, msg.sender, message);
        ...
        uint64 sequence = core_bridge.publishMessage(nonce, payload, 1);
    ...
    function processMyMessage(bytes memory VAA) public {
        ...
        require(myTrustedContracts[vm.emitterAddress][vm.emitterChainId], "Invalid emitter address!");
        ...
        (address intendedRecipient, uint16 _chainId, address sender, string memory message) = abi.decode(vm.payload, (address,uint16,address,string));
        ...
        require(intendedRecipient == address(this), "Not the intended receipient!");
...
// 0x000000000000000000000000fB7327Fe26aD52b693E38232E5D97F4892623075
// 0xfB7327Fe26aD52b693E38232E5D97F4892623075